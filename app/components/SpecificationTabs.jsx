    ...
    <section className=" py-[6rem] md:py-16">
      <div className="max-w-[1500] mx-auto">
        <Tabs defaultValue={specificationTabs[0].heading}>
          ...
          {specificationTabs.map((tabs, index) => (
            <TabsContent key={index} value={tabs.heading} className={"border-t-2 pt-8 md:pt-12 border-red-themed mt-10"}>
              <h2 className={`mont-font text-3xl lg:text-[2.8rem] text-text-black font-semibold text-center my-4 md:mb-8`}>{tabs.title}</h2>
              {tabs.description.map((desc, descIndex) => (
                <p key={descIndex} className="text-center px-12 lg:px-20 block mb-10">{desc}</p>
